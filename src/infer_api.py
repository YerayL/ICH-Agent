...
# Optional column fallbacks allow a single script to read either English or Chinese sheets.
...
class InferenceConfig:
    ...
    api_key: str = "EMPTY"
    output_dir: Path = Path("./Qwen3-30B-A3B_result")
    sheet_index: int = 0
...
def build_prompt_dataframe(file_path: Path, sheet_index: int) -> pd.DataFrame:
...
def _extract_message_choice(chat_response) -> Dict[str, str]:
...
def run_inference(config: InferenceConfig) -> None:
    prompts_df = build_prompt_dataframe(config.data_path, config.sheet_index)
...
def parse_args() -> InferenceConfig:
    parser = argparse.ArgumentParser(description="Batch infer Qwen3-30B-A3B for ICH prompts")
    parser.add_argument("data_path", type=Path, help="Path to the Excel file containing patient data")
    parser.add_argument("--model", default="/path/to/Qwen3-30B-A3B", help="Model identifier served by vLLM/OpenAI API")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("./Qwen3-30B-A3B_result"), help="Directory to save outputs")
