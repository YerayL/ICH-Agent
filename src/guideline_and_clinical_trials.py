...
class GuidelineRepository:
	"""Structured accessor for guideline and clinical trial text."""

	def __init__(self) -> None:
		self._guideline = GuidelineDocument(title="Guideline", body=GUIDELINE_TEXT)
		self._clinical_trials = GuidelineDocument(title="Clinical Trials", body=CLINICAL_TRIALS_TEXT)

	def get_guideline(self) -> GuidelineDocument:
		return self._guideline

	def get_clinical_trials(self) -> GuidelineDocument:
		return self._clinical_trials


# Keep legacy variable names for backward compatibility.
guideline = GUIDELINE_TEXT
clinical_trials = CLINICAL_TRIALS_TEXT
